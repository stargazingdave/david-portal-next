    ...
    return (
        <div className="flex flex-col items-center p-6 space-y-6">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {cards.map((card) => card.link ? (
                    <Link key={card.title} href={card.link} className="max-w-64 rounded-lg border border-secondary-foreground p-4 shadow-lg transition-colors hover:bg-foreground/10">
                        <h3 className="text-lg font-semibold">{card.title}</h3>
                        <p className="text-sm text-secondary-foreground">{card.description}</p>
                    </Link>
                ) : (
                    <article key={card.title} className="max-w-64 rounded-lg border border-muted-foreground/40 p-4 opacity-60">
                        <div className="mb-2 text-xs font-bold uppercase tracking-wide">Coming soon</div>
                        <h3 className="text-lg font-semibold">{card.title}</h3>
                        <p className="text-sm text-secondary-foreground">{card.description}</p>
                    </article>
                ))}
