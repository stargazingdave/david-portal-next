...
const SectionHeader = ({ children, id }: { children: React.ReactNode; id: string }) => (
    <h2 id={id} className="text-3xl text-blue-800 dark:text-blue-400 font-extrabold mt-4">
        {children}
    </h2>
);

const SectionSubheader = ({ children }: { children: React.ReactNode }) => (
    <h3 className="text-lg text-secondary-foreground font-semibold">{children}</h3>
);
...
const SectionList = ({ children, level = 0 }: { children: React.ReactNode; level?: 0 | 1 | 2 | 3 }) => {
    const levelClassName = level === 0 ? "mb-4" : level === 1 ? "pl-4" : level === 2 ? "pl-8" : "pl-12";
    return <ul className={`list-disc list-inside ${levelClassName}`}>{children}</ul>;
};

const Fig: React.FC<{ caption?: string; children: React.ReactNode }> = ({ caption, children }) => (
    <figure>
        {caption && (
            <figcaption className="text-lg text-muted-foreground mt-1">
                {caption}
            </figcaption>
        )}
        {children}
            ...
            <main className="max-w-4xl w-full">
                ...
                <div className="prose dark:prose-invert">
                    ...
                    <section aria-labelledby="espnow">
                        ...
                        <SectionParagraph>
                            Set both devices to the same channel. Use broadcast to get started, then switch to your peer's MAC for
                            unicast.
                        </SectionParagraph>
                    </section>
